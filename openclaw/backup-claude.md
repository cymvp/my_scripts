...
2. **rsync `--delete --no-links`**：`claude-global/` → `~/.claude/`
3. **rsync `--delete`**：`openclaw-mirror/` → `~/.openclaw/`
4. **rsync `--update`**：LaunchAgents plist（不能 `--delete`，因为该目录含其他不相关 plist）
5. **`claude-projects/`** → 各项目 `.claude/`：rsync `--delete`，canonical 前缀转回本机前缀以 resolve 实际项目路径

> restore 阶段使用 `--delete` 是因为经过 Phase 3 pull+merge 后，repo 是两台机器数据合并后的权威状态。repo 中不存在的文件应该也从本地移除。
...
另一台机器删了文件并 sync → git 记录删除 → 本机 pull 后 repo 里没了 → restore `--delete` 从本地也删除。**无需手动操作。**

### 本地删除 → 需要额外步骤

仅从 `~/.claude/` 删除文件是不够的。因为 backup 阶段的 rsync 不带 `--delete`（为了保护远端数据），repo 里的副本不会被删除，下次 restore 会把文件恢复回来。

**正确做法**：同时删除本地和 repo 中的对应文件：

```bash
# 例：删除 ~/.claude/notes/old-file.md
rm ~/.claude/notes/old-file.md
rm ~/projects/migrate_to_new_device/claude-backups/claude-global/notes/old-file.md
# 然后运行 sync，git add -A 会检测到 repo 中的删除并提交
```

### 为什么 backup 不用 --delete

backup 在 pull 之前执行。如果 backup 带 `--delete`，一台长时间未同步的机器会在 commit 前就把 repo 里其他机器的新文件全部删掉，导致 git merge 无法看到这些文件，造成数据丢失。

### 为什么 restore 可以用 --delete

restore 在 pull 之后执行。此时 repo 经过 git merge，包含两台机器数据的合并结果。restore `--delete` 让本地精确匹配 repo 的合并状态。只有 git merge 过程中被删除的文件（即另一台机器主动删除的）才会从本地消失。

## 测试

测试脚本：`./test-backup-claude.sh`

覆盖场景：
1. 基本同步（A→B）
2. 双向同步（A↔B）
3. 远端删除传播（A 删 → B 同步 → B 也没了）
4. 仅本地删除恢复（文件复活）
5. 正确删除（本地+repo 同时删）
6. 过期机器同步（不丢远端新文件）
7. `--update` 保护较新文件
